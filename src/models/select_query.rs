...
impl TryFrom<&SetExpr> for SelectQuery {
    ...
    fn try_from(value: &SetExpr) -> Result<Self, Self::Error> {
        if let Select(select) = value {
            ...
            let constraints = match select.selection.as_ref(){
                Some(r) => Some(r.clone()),
                None => None
            };
                ...
                Expr::BinaryOp { left, op, right } => {
                    match op {
                        ...
                        BinaryOperator::And => {
                            ...
                            println!("Doing the and operation: left: {:?}, right: {:?}", left, right);
                        ...
                        BinaryOperator::Or => match (evaluate(values, Some(*left)), evaluate(values, Some(*right))){
                            (Ok(v1), Ok(v2)) => Ok(v1 || v2),
                            _ => Ok(false)
                        },
                        _ =>  Err(SelectQueryError::IsNotSupportedError())
            ...
            match statement {
                Query(query) => {
                    ...
                    let values = HashMap::from([
                        ("x".to_string(), TypeValue::NumberValueType(NumberValueType{value: 1.0})),
                        ("u".to_string(), TypeValue::NumberValueType(NumberValueType{value: 2.0})),
                        ("y".to_string(), TypeValue::NumberValueType(NumberValueType{value: 3.0}))
                    ]);
            ...
            match statement {
                Query(query) => {
                    ...
                    let values = HashMap::from([
                        ("x".to_string(), TypeValue::NumberValueType(NumberValueType{value: 1.0})),
                        ("u".to_string(), TypeValue::NumberValueType(NumberValueType{value: 2.0})),
                        ("y".to_string(), TypeValue::NumberValueType(NumberValueType{value: 4.0}))
                    ]);
            ...
            match statement {
                Query(query) => {
                    ...
                    let values = HashMap::from([
                        ("x".to_string(), TypeValue::NumberValueType(NumberValueType{value: 1.0})),
                        ("u".to_string(), TypeValue::NumberValueType(NumberValueType{value: 2.0})),
                        ("y".to_string(), TypeValue::NumberValueType(NumberValueType{value: 2.0}))
                    ]);
